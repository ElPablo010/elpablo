...
- De site is meertalig (NL/EN/ES) met NL als hoofdtaal; hreflang-alternates en de sitemap zijn al ingericht. Richt je advies op de Nederlandse content — adviseer geen hreflang-setup die er al is.
...
- De site is meertalig (NL/EN/ES) met NL als hoofdtaal. Werk uitsluitend op de Nederlandse versie; de vertalingen worden apart beheerd. Stel dus geen acties voor die over vertalen of over /en- en /es-URL's gaan.
...
PROMPT;
     ...
     * Binnen de hoofdtaal — de SEO-acties werken enkel op NL-content.
     */
     ...
     * builder-contract van dit project (`text` = heading+body, `faq` = items).
     *
     ...
     * generieke builder-blokken (hero → text → faq → cta). De hero- en
    ...
    {
        $cta = $this->homepage()['cta'] ?? null; // ['label','href','link_type','page_id'] of null
        $ctaButton = $cta ? [[
            'label' => $cta['label'],
            'variant' => 'primary',
            'link_type' => $cta['link_type'] ?? 'url',
    ...
    protected function homepage(): array
    {
        ...
        // Deze site is meertalig: er is één homepage per taal. Neem bewust de
        // NL-versie — de SEO-acties leveren NL-content.
        $page = Page::where('is_homepage', true)->where('locale', Locale::DEFAULT)->first();
                ...
                if (in_array($s->section_type, ['hero', 'cta'], true)) {
                    foreach ($c['ctas'] ?? [] as $cta) {
                        ...
                        if ($href = $this->resolveCtaHref($cta)) {
                            // link_type/page_id mee overnemen: de CtaLinkSchema in de
                            // admin leest die om het juiste veld te tonen. Zonder die
                            // sleutels valt de knop in de builder terug op "Pagina"
                            // met een lege selectie, terwijl de link wel degelijk werkt.
                            $anchors['cta'] ??= [
                                'label' => (string) $cta['label'],
                                'href' => $href,
                                'link_type' => ($cta['link_type'] ?? null) === 'page' && ! empty($cta['page_id']) ? 'page' : 'url',
                                'page_id' => ($cta['link_type'] ?? null) === 'page' ? ($cta['page_id'] ?? null) : null,
                            ];
                            break;
                        }
                    }
    ...
    /**
     * Zoek een bestaande pagina op slug ("/" of "home" → homepage). Altijd binnen
     * de hoofdtaal: EN/ES delen dezelfde slugs, dus zonder locale-scope zou een
     * actie op de verkeerde vertaling kunnen landen.
     */
    protected function resolvePage(?string $slug): ?Page
    {
        ...
        if ($slug === '/' || strtolower($slug) === 'home') {
            return Page::where('is_homepage', true)->where('locale', Locale::DEFAULT)->first();
        }

        return Page::where('slug', ltrim($slug, '/'))->where('locale', Locale::DEFAULT)->first();
    }
