 ...
 *   - `text`-sectie: { heading, body }  (body = rich-text HTML)
...
class SeoActionApplier
{
    ...
    /**
     * Garandeer een unieke slug binnen de hoofdtaal. `pages` heeft een unique op
     * [locale, slug]: dezelfde slug mag dus in EN/ES bestaan en hoeft hier niet
     * mee te tellen — anders zouden we nodeloos "-2" achter de slug plakken.
     */
    protected function uniqueSlug(string $slug): string
    {
        ...
        $base = $slug;
        $i = 2;
        while (Page::where('locale', Locale::DEFAULT)->where('slug', $slug)->exists()) {
            $slug = "{$base}-{$i}";
            $i++;
        }
