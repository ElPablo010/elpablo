...
@php
    ...
    // Een view die zelf weet waar zijn record in de admin woont (bv. een
    // event) geeft dat mee via editUrl. Anders: staat er een pagina op het
    // scherm, spring dan naar haar bewerkscherm; anders (404, preview) naar
    // het paginaoverzicht.
    $label = match (true) {
        (bool) $editUrl => 'Bewerk in de admin',
        (bool) $page?->exists => 'Bewerk deze pagina in de admin',
        default => 'Naar het paginaoverzicht in de admin',
    };
    $href = $editUrl ?? ($page?->exists
    ...
    <div class="pointer-events-none fixed inset-x-0 top-28 z-40 px-4">
        <div class="mx-auto flex max-w-7xl justify-end">
            ...
            >
                ...
                <span class="sr-only">{{ $editUrl || $page?->exists ? $label : 'Naar de admin' }}</span>
            </a>
        </div>
    </div>
@endif
