...
beforeEach(function () {
    ...
    Setting::set('seo_target_domain', 'el-pablo.be');
});
...
{
    $page = Page::create(array_merge(['locale' => 'nl', 'published' => true], $attributes));
...
function seoGuardPageWithFaq(string $slug, array $questions): Page
{
    ...
    $page->sections()->create([
        ...
        'content' => [
            ...
            'items' => array_map(fn ($q) => ['question' => $q, 'answer' => 'Bij El Pablo.'], $questions),
        ],
    ]);
...
it('filters out a question that is already answered on the page', function () {
    seoGuardPageWithFaq('tapas', ['Waar vind ik jullie tapasbar?']);
    ...
    seoGuardFakeProposes([[
        ...
        'target_slug' => 'tapas',
        'faq' => [
            ['question' => 'Waar vind ik jullie tapasbar in de buurt van Antwerpen?', 'answer' => 'In hartje stad.'],
            ['question' => 'Kan ik glutenvrij eten bij jullie?', 'answer' => 'Zeker, geef het door.'],
        ],
    ]]);
    ...
    expect($actions)->toHaveCount(1)
        ...
        ->toBe(['Kan ik glutenvrij eten bij jullie?']);
});
...
it('drops the whole action when no genuinely new questions remain', function () {
    ...
    seoGuardFakeProposes([[
        ...
        'problem' => 'Vragen zonder antwoord.',
        'target_slug' => 'tapas',
        'faq' => [['question' => 'Waar vind ik jullie tapasbar in Antwerpen?', 'answer' => 'In hartje stad.']],
    ]]);
...
it('refuses new questions on a full FAQ block and caps additions below it', function () {
    seoGuardPageWithFaq('vol', array_map(
        fn ($i) => "Bestaande vraag nummer {$i} over eten?",
        range(1, SeoAdvisorService::FAQ_MAX_QUESTIONS)
    ));
    seoGuardPageWithFaq('bijna-vol', array_map(
        fn ($i) => "Andere bestaande vraag nummer {$i} over drinken?",
        range(1, SeoAdvisorService::FAQ_MAX_QUESTIONS - 2)
    ));

    $newFaq = [
        ['question' => 'Hebben jullie een terras?', 'answer' => 'Ja.'],
        ['question' => 'Kan ik met de fiets parkeren?', 'answer' => 'Ja.'],
        ['question' => 'Zijn honden welkom?', 'answer' => 'Ja.'],
    ];
    seoGuardFakeProposes([
...
it('refuses a create_page for a keyword an existing page already covers', function () {
    seoGuardPage(['title' => 'Tapasbar in Antwerpen', 'slug' => 'tapasbar-antwerpen'], 7);
    ...
    $createPage = fn (string $keyword, string $slug) => [
        ...
        'why_html' => '<p>Omdat het lekker is.</p>',
    ];
    seoGuardFakeProposes([
        $createPage('tapas antwerpen', 'tapas-antwerpen'),
        $createPage('paella catering mechelen', 'paella-catering-mechelen'),
    ]);

    $actions = app(SeoAdvisorService::class)->generateActions(seoGuardContext());

    expect($actions)->toHaveCount(1)
        ->and($actions[0]['source_keyword'])->toBe('paella catering mechelen');
});

it('leaves an existing meta alone on a recently touched page but fills empty fields', function () {
    seoGuardPage(['title' => 'Tapas', 'slug' => 'tapas', 'meta_title' => 'Tapas | El Pablo'], 7);
    ...
    seoGuardFakeProposes([[
        ...
        'problem' => 'Zwakke meta.',
        'target_slug' => 'tapas',
        'meta_title' => 'Nieuwe titel | El Pablo',
...
it('feeds the prompt the existing FAQ questions and recency markers', function () {
    seoGuardPageWithFaq('tapas', ['Waar vind ik jullie tapasbar?']);
    seoGuardPage(['title' => 'Paella', 'slug' => 'paella'], SeoAdvisorService::RECENT_DAYS + 30);
    ...
    Http::assertSent(function ($request) {
        ...
        return str_contains($prompt, 'Bestaande FAQ-vragen per pagina')
            && str_contains($prompt, 'Waar vind ik jullie tapasbar?')
            && preg_match('/tapas.*NIEUW sinds/', $prompt)
            && ! preg_match('/paella.*(NIEUW|RECENT)/', $prompt);
    });
});
